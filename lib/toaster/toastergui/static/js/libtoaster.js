...
var libtoaster = (function () {
  ...
  /* ### WIND_RIVER_EXTENSION_BEGIN ### */
  function _addRmWRTemplate(wrtemplateObj, add, doneCb){
    if (add === true) {
      var editData = { wrtemplateAdd : wrtemplateObj.id };

      libtoaster.editCurrentProject(editData, function () {
        doneCb([]);
      }, function () {
        console.warn ("Adding wrtemplate to project failed");
        doneCb(null);
      });
    } else if (add === false) {
      var editData = { wrtemplateDel : wrtemplateObj.id };

      libtoaster.editCurrentProject(editData, function () {
        doneCb([]);
      }, function () {
        console.warn ("Removing wrtemplate from project failed");
        doneCb(null);
      });
    }
  }

  function _makeWRTemplateAddRmAlertMsg(wrtemplate, add) {
    var alertMsg;

    if (add === true) {
      alertMsg = $("<span>You have added <strong>1</strong> template to your project: <a class=\"alert-link\" id=\"wrtemplate-affected-name\"></a></span></span>");
    } else {
      alertMsg = $("<span>You have removed <strong>1</strong> template from your project: <a class=\"alert-link\" id=\"wrtemplate-affected-name\"></a></span>");
    }

    alertMsg.children("#wrtemplate-affected-name").text(wrtemplate.name);
    alertMsg.children("#wrtemplate-affected-name").attr("href", wrtemplate.layerdetailurl);
  ...
  */
  ...
  return {
    ...
    /* ### WIND_RIVER_EXTENSION_BEGIN ### */
    addRmWRTemplate : _addRmWRTemplate,
    makeWRTemplateAddRmAlertMsg : _makeWRTemplateAddRmAlertMsg,
    /* ### WIND_RIVER_EXTENSION_END ### */
  };
})();
